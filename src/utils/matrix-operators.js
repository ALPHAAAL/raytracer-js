...
export default class MatrixOperators {
    ...
    static normalAt(shape, worldPoint) {
        ...
        if (shape.getTransform()) {
            ...
            objectPoint = MatrixOperators.multiply(MatrixOperators.inverse(shape.getTransform()), worldPoint);
            const objectNormal = shape.normalAt(objectPoint);
            ...
            const worldNormal = MatrixOperators.multiply(MatrixOperators.transpose(MatrixOperators.inverse(shape.getTransform())), objectNormal);
            const [x, y, z] = worldNormal.getValues();

            return Operators.normalize(new Vector(x, y, z));
        }

        return shape.normalAt(objectPoint);
    }
}
